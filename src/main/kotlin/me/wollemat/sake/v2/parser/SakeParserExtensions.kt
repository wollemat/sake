...
fun parseIfExpression(expressions: List<SakeParser.ExprContext>): ExpressionNode = when (expressions.size) {
    ...
    2 -> throw IllegalStateException()
        ...
        expressions[0].toAST(),
        expressions[1].toAST(),
        parseIfExpression(expressions.drop(2))
    )
}
...
fun SakeParser.ExprContext.toAST(): ExpressionNode = when (this) {
    ...
    is SakeParser.ConcatExpressionContext -> ConcatNode(expr(0).toAST(), expr(1).toAST())
    is SakeParser.ArrayExpressionContext -> ArrayNode(expr(0).toAST(), expr(1).toAST())
    ...
    is SakeParser.ArgumentApplicationExpressionContext ->
