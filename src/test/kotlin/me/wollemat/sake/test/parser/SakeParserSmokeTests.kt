...
class SakeParserSmokeTests {
    ...
    fun `abs function smoke test`() {
        ...
        assertEquals(
            "AbstractSyntaxTree(funcs=[FunctionNode(id=abs, params=[x], expr=IfNode(cond=LtNode(expr1=VariableNode(id=x), expr2=IntegerNode(num=0)), expr1=NegNode(expr=VariableNode(id=x)), expr2=VariableNode(id=x)))])",
            ast.toString()
        )
    }
